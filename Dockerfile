...
RUN pip3 install tensorflow matplotlib numpy pillow six scipy
...
RUN make -j4
RUN make install
RUN cp /opencv/build/lib/python3/cv2.cpython-*-gnu.so /usr/local/lib/python3.8/dist-packages/cv2.so
WORKDIR /tf_tracking
